-- ═══════════════════════════════════════════════════════════════════════════
-- SOLANA QUEST — refresh: answer positions, a real bank, and a day that
-- builds itself
-- ═══════════════════════════════════════════════════════════════════════════
-- Project: qxejdpvjggqjqoydujjd.supabase.co   ·   run in the SQL Editor
-- Requires supabase-quiz.sql to have been applied first.
--
-- Three things were wrong in production on 2026-09-05:
--
--   1. Every one of the 20 seeded questions had correct_index = 0, and the app
--      does not shuffle. The right answer was always button A. Anyone who
--      noticed scored five out of five daily without knowing any Solana.
--
--   2. The bank was too small for its own picker. A day needs 2 easy, 2 medium
--      and 1 hard, and build_quiz_day() refused to reuse anything from the past
--      60 days. With 6 easy questions in the bank that is three days of quiz.
--
--   3. Nothing ever called build_quiz_day(). It ran once by hand at the bottom
--      of supabase-quiz.sql and never again, so the last issue was 2026-08-24
--      and quiz_today has been empty for twelve days — while the biggest card
--      on the home screen invites everyone to play it.
--
-- PART 1 fixes the picker, PART 2 exposes a safe way to trigger it, PART 3
-- re-seats the answers, PART 4 tops the bank up, PART 5 builds today.
-- Running the whole file twice is harmless.
--
-- FORMATTING, deliberately: this file contains ZERO single-quote characters
-- and zero semicolons inside any text. Every string is dollar-quoted. An
-- earlier version failed in the Supabase SQL Editor with a nonsense error
-- pointing at a word buried in an English sentence, which means something in
-- the path between the editor and Postgres was losing track of where string
-- literals begin and end. Rather than keep guessing at that, the file no
-- longer gives it anything to lose track of: with no quotes in the content and
-- no semicolons inside it, splitting on semicolons lands exactly on statement
-- boundaries whether or not dollar quoting is understood.
-- ═══════════════════════════════════════════════════════════════════════════
