...
| # | Шаг | Кто | Статус |
|---|---|---|---|
| 0.1 | Установить текущий APK на телефон, отыграть 10 минут, дать список поломок | **ты** | ✅ 2026-08-24 |
| 0.2 | Починить найденное | я | ✅ 2026-08-24 |
| 0.3 | Поднять версию (1.1.4 → 1.2.0) и собрать релизный multi-arch APK | я | ⏸ отложено |
| 0.4 | Опубликовать в dApp Store | ты запускаешь | 🟡 в процессе |
| 0.5 | Скачать опубликованную версию из стора, проверить что ставится и работает | **ты** | ⬜ |

**0.3 отложен намеренно.** Готовый подписанный APK от 2026-08-17 (v1.1.4,
versionCode 15, 139.7 МБ) уже содержит весь Лабиринт. Пересборка требует
JDK + Android SDK + NDK, а на этой машине их нет — при скорости ~12 КБ/с это
полтора-два дня закачки. Публикуем готовое сейчас, версию поднимем следующим
билдом вместе с Договорами и исправлением платежей.

### 💰 Платежи проверены вживую (2026-08-24) — ВПЕРВЫЕ

Три месяца платёжный путь ни разу не запускался с реальными деньгами.
Проверен — **работает**.

Первая попытка упала с «Simulation failed» и дампом логов, где все программы
вернули `success`. Причина оказалась не в коде: на кошельке было ровно 0.01 SOL
при цене 0.01 SOL, а Solana требует оставить неснижаемый остаток 890 880
лампортов. Не хватило 0.0009 SOL.

Настоящий баг был в другом — **проверки баланса в коде не было вообще**, и
игрок получал сырой дамп симуляции в Alert. Исправлено в `lib/solanaMobile.ts`:
проверка баланса до отправки и человеческие тексты ошибок. Уедет следующим
билдом.

**Вывод для экономики: деньги ходят.** Это снимает главную неизвестность —
дальше вопрос только в числе игроков, а не в том, работает ли монетизация.
